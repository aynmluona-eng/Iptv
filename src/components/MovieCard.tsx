  ...
  const handleMouseEnter = () => {
    ...
    if (!description && !loading) {
      hoverTimeoutRef.current = setTimeout(async () => {
        setLoading(true);
        try {
          const data = await fetchXtreamApi(credentials, 'get_vod_info', { vod_id: movie.stream_id });
          if (data?.info?.plot) {
      ...
      }, 500); // 500ms delay before fetching to avoid spam
    }
  };
    ...
    >
      <div className="aspect-[2/3] relative rounded-sm overflow-hidden mb-3 border border-white/5 bg-panel group-hover:border-brand transition-colors">
        ...
        {movie.rating && movie.rating > 0 ? (
          <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md px-2 py-1 flex items-center gap-1 rounded-md border border-white/10 z-10">
            ...
            <span className="text-[10px] font-bold font-mono text-white pt-0.5">{Number(movie.rating).toFixed(1)}</span>
          </div>
        ) : null}
      </div>
      ...
      <AnimatePresence>
        {isHovered && description && (
          ...
          >
            <h4 className="text-brand font-bold text-sm mb-1">{movie.name}</h4>

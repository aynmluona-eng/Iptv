...
export default function MovieDetails({
  ...
  useEffect(() => {
    if (id) {
      const favs = JSON.parse(
        localStorage.getItem("xtream_favorites_movies") || "[]",
      );
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const data = await fetchXtreamApi(credentials, "get_vod_info", {
          vod_id: id,
        });
        if (data && data.info) {
          ...
          if (catId) {
            ...
            if (Array.isArray(movies)) {
              setRelated(
                movies
                  .filter((m) => String(m.stream_id) !== String(id))
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-xs md:text-sm text-gray-200 mb-6 bg-black/40 py-2.5 px-5 rounded-full border border-white/10 inline-flex backdrop-blur-md shadow-inner">
                {movieInfo.releasedate && (
                  <span className="font-mono">{movieInfo.releasedate}</span>
                )}
                {movieInfo.rating && (
                  <span className="flex items-center gap-1.5 text-yellow-400 font-bold">
                    <Star size={16} className="fill-yellow-400" />{" "}
                    {Number(movieInfo.rating).toFixed(1)}
                  </span>
                )}
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                <button
                  onClick={() =>
                    navigate(
                      `/player/movie/${id}?ext=${movieData?.container_extension || "mp4"}`,
                    )
                  }
                ...
                {movieInfo.youtube_trailer && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="bg-red-600/20 text-red-500 border border-red-600/50 px-8 py-4 rounded-sm font-bold text-lg hover:bg-red-600/30 transition-all flex items-center justify-center gap-3"
                  >
                    <Play fill="currentColor" size={20} /> اعلان ترويجي
                  </a>
                )}

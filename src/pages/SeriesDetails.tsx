...
export default function SeriesDetails({
  ...
  useEffect(() => {
    if (id) {
      const favs = JSON.parse(
        localStorage.getItem("xtream_favorites_series") || "[]",
      );
  ...
  useEffect(() => {
    ...
    const loadSeriesInfo = async () => {
      try {
        const data = await fetchXtreamApi(credentials, "get_series_info", {
          series_id: id,
        });
        if (data) {
          ...
          if (data.info?.category_id) {
            const seriesList = await fetchXtreamApi(credentials, "get_series", {
              category_id: data.info.category_id,
            }).catch(() => []);
            if (Array.isArray(seriesList)) {
              setRelated(
                seriesList
                  .filter((s) => String(s.series_id) !== String(id))
  ...
  const castArray = seriesInfo.cast
    ? seriesInfo.cast
        .split(",")
        .map((c: string) => c.trim())
        .filter(Boolean)
        ...
        <div className="relative min-h-[70vh] flex flex-col justify-end pt-32 pb-16 px-6 md:px-12">
          ...
          <div className="absolute inset-0 z-0">
            {seriesInfo.backdrop_path && seriesInfo.backdrop_path.length > 0 ? (
              <img
                src={seriesInfo.backdrop_path[0]}
                alt="backdrop"
          ...
          <button
            onClick={() => navigate(-1)}
            className="absolute top-8 right-6 md:right-12 z-20 w-12 h-12 bg-black/40 backdrop-blur-md hover:bg-brand text-white hover:text-black border border-white/10 rounded-full flex items-center justify-center transition-all"
          >
            <ArrowLeft size={24} />
          </button>
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row gap-8 lg:gap-12 items-center md:items-end">
            <motion.div
              ...
              className="w-48 md:w-64 lg:w-72 shrink-0 rounded-sm overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-2 border-white/10 bg-panel group"
            >
              <ImageWithFallback
                src={seriesInfo.cover}
                alt={seriesInfo.name}
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-xs md:text-sm text-gray-200 mb-6 bg-black/40 py-2.5 px-5 rounded-full border border-white/10 inline-flex backdrop-blur-md shadow-inner">
                {seriesInfo.releaseDate && (
                  <span className="font-mono">{seriesInfo.releaseDate}</span>
                )}
                {seriesInfo.rating && (
                  <span className="flex items-center gap-1.5 text-yellow-400 font-bold">
                    <Star size={16} className="fill-yellow-400" />{" "}
                    {Number(seriesInfo.rating).toFixed(1)}
                  </span>
                )}
                {episodes.length > 0 && <span>{episodes.length} حلقة</span>}
              </div>

              <p className="text-gray-300 text-sm md:text-base lg:text-lg leading-relaxed max-w-3xl mb-8 drop-shadow-md">
              ...
              {episodes.length > 0 && (
                <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                  <button
                    onClick={() =>
                      navigate(
                        `/player/series/${episodes[0]?.id}?ext=${episodes[0]?.container_extension || "mp4"}`,
                      )
                    }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {episodes.map((ep, idx) => (
                <motion.div
                  ...
                  onClick={() =>
                    navigate(
                      `/player/series/${ep.id}?ext=${ep.container_extension || "mp4"}`,
                    )
                  }
                >
                  ...
                  <div className="flex-1 min-w-0 pr-1">
                    <div className="text-[10px] font-bold text-gray-300 mb-1 uppercase tracking-wider bg-brand/10 inline-block px-2 py-0.5 rounded">
                      الموسم {ep.season} - حلقة {ep.episode_num}
                    </div>
                    <h3 className="text-white text-sm font-bold truncate group-hover:text-gray-300 transition-colors">
                      {ep.title}
                    </h3>
                  </div>
                </motion.div>

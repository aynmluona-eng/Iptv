...
export default function Live({
    ...
    const loadData = async () => {
      try {
        const [cats, chs] = await Promise.all([
          ...
          fetchXtreamApi(credentials, "get_simple_data_table")
            .catch(() => null)
            .then((res) =>
              res && res.length > 0
                ? res
  ...
  const validCategories = useMemo(() => {
    return categories.filter(
      (c) => channelsByCategory[c.category_id]?.length > 0,
    );
        ...
        <header className="mb-6 shrink-0 md:flex md:justify-between md:items-center">
          <div className="flex justify-between items-center mb-4 md:mb-0">
            <div className="flex items-center gap-3">
              <button
                onClick={() =>
                  activeCategory ? setActiveCategory(null) : navigate("/")
                }
                className="w-10 h-10 bg-white/5 hover:bg-brand hover:text-black rounded-sm flex items-center justify-center transition-colors"
              >
                <ArrowLeft size={24} />
              </button>
          ...
          <div
            className={clsx(
              "relative w-full md:w-auto md:block",
              ...
              {filteredChannels.slice(0, visibleCount).map((stream, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {stream.epg_title && (
                      <p
                        className="text-xs text-brand truncate px-2 mt-1 w-full max-w-full text-center"
                        dir="auto"
                      >
                        {decodeEpg(stream.epg_title)}
                      </p>
                    )}

                    <button
                      onClick={(e) =>
                        toggleFavorite(e, String(stream.stream_id))
                      }
            ...
            <div className="flex flex-col gap-10">
              ...
              {validCategories.slice(0, visibleCount).map((cat) => (
                <section key={cat.category_id}>
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 styled-scrollbar snap-x">
                    {channelsByCategory[cat.category_id]
                      ?.slice(0, 15)
                      .map((stream, idx) => {
                        const isFav = favorites.has(String(stream.stream_id));
                        return (
                          <div
                            ...
                            className="w-[140px] md:w-[160px] shrink-0 snap-start group cursor-pointer"
                            onClick={() =>
                              navigate(`/live/${stream.stream_id}`)
                            }
                          >
                            <div className="aspect-square relative rounded-sm overflow-hidden mb-2 border border-white/5 bg-panel group-hover:border-brand transition-colors flex items-center justify-center p-4">
                              <ImageWithFallback
              ...
              {visibleCount < validCategories.length && (
                <div
                  ref={observerRef}
                  className="col-span-full h-20 flex items-center justify-center mt-6"
                >
                  <div className="w-8 h-8 rounded-full border-2 border-white/10 border-t-brand animate-spin"></div>
                </div>
              )}

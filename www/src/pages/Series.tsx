...
import { Search, Star, MonitorPlay, ArrowLeft, ArrowLeftCircle, Play, Filter } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import clsx from 'clsx';

export default function Series({ credentials }: { credentials: XtreamCredentials }) {
  const [categories, setCategories] = useState<any[]>([]);
  const [seriesList, setSeriesList] = useState<any[]>([]);
  const [seriesByCategory, setSeriesByCategory] = useState<Record<string, any[]>>({});
    ...
    const loadData = async () => {
      try {
        const [cats, series] = await Promise.all([
          fetchXtreamApi(credentials, 'get_series_categories').catch(() => []),
  ...
  const availableYears = useMemo(() => {
    ...
    seriesList.forEach(s => {
      const match = s.name?.match(/\b((?:19|20)\d{2})\b/);
  ...
  const filteredSeries = useMemo(() => {
    return seriesList.filter(s => {
      ...
      const matchCategory = activeCategory ? s.category_id === activeCategory : true;
      let matchYear = true;
      if (selectedYear) {
         const yearMatch = s.name?.match(/\b((?:19|20)\d{2})\b/);
         matchYear = yearMatch ? yearMatch[1] === selectedYear : false;
      }
      return matchSearch && matchCategory && matchYear;
    });
  }, [seriesList, search, activeCategory, selectedYear]);

  // Mock continue watching for now
  const continueWatching = useMemo(() => seriesList.slice(0, 10), [seriesList]);

  // Categories with series
  const validCategories = useMemo(() => {
    return categories.filter(c => seriesByCategory[c.category_id]?.length > 0);
  }, [categories, seriesByCategory]);
          ...
          <div className="flex justify-between items-center mb-4 md:mb-0">
             <div className="flex items-center gap-3">
               ...
               <h1 className="text-2xl font-bold tracking-tight">
                 {activeCategory 
                   ? `مسلسلات / ${categories.find(c => c.category_id === activeCategory)?.category_name || ''}` 
                   : 'المسلسلات'
                 }
               </h1>
             </div>
        ...
        <div className="flex-1 overflow-y-auto pb-6 styled-scrollbar">
          {loading ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
              {filteredSeries.slice(0, visibleCount).map((series, idx) => (
                <SeriesCard key={series.series_id} series={series} credentials={credentials} idx={idx} />
              ))}
            ...
            <div className="flex flex-col gap-10">
              ...
              {continueWatching.length > 0 && (
                <section>
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 styled-scrollbar snap-x">
                    {continueWatching.map((series, idx) => (
                      <div 
                      ...
                      >
                        ...
                        <h3 className="text-white text-sm font-bold line-clamp-1">{series.name}</h3>
                      </div>
                    ))}
                  </div>
                </section>
              ...
              {validCategories.slice(0, visibleCount).map((cat) => (
                <section key={cat.category_id}>
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 styled-scrollbar snap-x">
                    {seriesByCategory[cat.category_id]?.slice(0, 15).map((series, idx) => (
                      <SeriesCard 
                        key={series.series_id} 
                        series={series} 

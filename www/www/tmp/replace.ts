...
files.forEach(file => {
  ...
  content = content.replace(/from-[#1A2235] to-[#131824]/g, 'from-panel to-dark');
  content = content.replace(/bg-\[#1A2235\]/g, 'bg-panel');
  content = content.replace(/bg-\[#131824\]\/50/g, 'bg-panel');
  content = content.replace(/bg-\[#131824\]\/80/g, 'bg-panel');
